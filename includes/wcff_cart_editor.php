...
class wcff_cart_editor {
    ...
    public function render_fields_data($_html, $_cart_item, $_cart_item_key, $_is_review_table) {
        ...
        if (isset($this->cart_item['product_id'])) {
            ...
            if( isset( $this->cart_item['variation_id'] ) && !empty( $this->cart_item['variation_id'] ) && $this->cart_item['variation_id'] != 0 ) {
                $this->product_field_groups = array_merge( $this->product_fields, wcff()->dao->load_fields_groups_for_product($this->cart_item['variation_id'], 'wccpf', $template, 'cart-page' ));                
            }
            ...
            if (isset($this->cart_item["quantity"])) {
            	...
            	if ($this->determine_fields_there_to_render()) {
            		...
            		/* Fields wrapper end */
            		$this->meta_html .= '</div>';
            	}           else { error_log("No fields to render"); }
    ...
    public function split_order_item( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
        ...
        $this->product_fields = wcff()->dao->load_fields_groups_for_product($product_id, 'wccpf', $template, "any");
        $this->admin_fields = wcff()->dao->load_fields_groups_for_product($product_id, 'wccaf', $template, "any");
        
        if( isset( $variation_id ) && !empty( $variation_id ) && $variation_id != 0 ){
            $this->product_fields = array_merge( $this->product_fields, wcff()->dao->load_fields_groups_for_product($variation_id, 'wccpf', 'cart-page' ));
            $this->admin_fields = array_merge( $this->admin_fields, wcff()->dao->load_fields_groups_for_product($variation_id, 'wccaf', 'cart-page') );
        }
    ...
    private function render_product_fields_data() {
		$field_editors = "";
		foreach ( $this->product_fields as $group ) {
		    
		    if (count($group["fields"]) > 0) {
		        foreach ($group["fields"] as $field) {		    
		            $field ["visibility"] = isset ( $field ["visibility"] ) ? $field ["visibility"] : "yes";
                    if ($field ["visibility"] == "yes" && isset ( $this->cart_item [$field ["key"]] )) {
                        $field_editors .= $this->render_data ( $field, $this->cart_item [$field ["key"]] );
                    }	            
		        }
		    }
				...
				foreach ( $this->product_fields as $group ) {
				    ...
				    if (count($group["fields"]) > 0) {
				        ...
				        foreach ($group["fields"] as $field) {			                
		                    ...
		                    $field ["visibility"] = isset ( $field ["visibility"] ) ? $field ["visibility"] : "yes";
		                    if ($field ["cloneable"] == "yes" && $field ["visibility"] == "yes" && isset ( $this->cart_item [$field ["key"] . "_" . $i] )) {
		                        $field_editors .= $this->render_data ( $field, $this->cart_item [$field ["key"] . "_" . $i], ("_" . $i) );
		                    }
				        }
				    }
				}
    ...
    private function render_admin_fields_data($_index = 0, $_title_displayed = true) {
    	$field_editors = "";
        if ($_index == 0) {
            foreach ($this->admin_fields as $group) {
                
                if (count($group["fields"]) > 0) {
                    foreach ($group["fields"] as $afields) {
                        if (is_array($afields) && count($afields)) {
                            foreach ($afields as $key => $afield) {
                                ...
                                if ($afield["visibility"] == "yes" && isset($this->cart_item[$afield["key"]])) {
                                    $field_editors .= $this->render_data($afield, $this->cart_item[$afield["key"]]);
                                }
                            }
                        }
                    }
                            ...
                            if (is_array($afields) && count($afields)) {
                                ...
                                if (!$_title_displayed) {
                                    ...
                                    foreach ($afields as $key => $afield) {
                                        ...
                                        if ($afield["cloneable"] == "yes" && $afield["visibility"] == "yes" && isset($this->cart_item[$afield["key"] . "_" . $_index])) {
                                            $meta_there = true;
                                        }
                                    }
                                    if ($meta_there) {
                                ...
                                foreach ($afields as $key => $afield) {
                                    ...
                                    $afield["cloneable"] = isset($afield["cloneable"]) ? $afield["cloneable"] : "yes";
                                    if ($afield["cloneable"] == "yes" && $afield["visibility"] == "yes" && isset($this->cart_item[$afield["key"] . "_" . $_index])) {
                                        $field_editors .= $this->render_data($afield, $this->cart_item[$afield["key"] . "_" . $_index], ("_" . $_index));
                                    }
                                }
                            }
                        }                        
		...
		if ($_field ["type"] != "file" && $_field ["type"] != "checkbox" && $_field ["type"] != "colorpicker") {
			...
			if ($is_multi_file == "yes") {
				...
				if ($_field ["img_is_prev"] == "yes" && @getimagesize ( $fobj ["url"] )) {
					...
					$meta_html .= '<li class="wcff_field_cart_updater_clone" data-field="' . $_field ["key"] . '" data-fieldkey="' . ($_field ["key"] . $_index) . '" data-productid="' . esc_attr ( $this->cart_item ["product_id"] ) . '" data-itemkey="' . esc_attr ( $this->cart_item_key ) . '">' . wp_kses_post ( stripslashes ( $path_parts ["basekey"] ) ) . '</li>';
				}
			}
		}
		$meta_html .= '</ul>';
    ...
    public function update_field_value($_payload) {
        ...
        if (!$validate["status"]){
            ...
            if ($_payload["data"]["field_type"] != "file"){
            	if( isset( WC()->cart->cart_contents[$_payload['cart_item_key']]["wccpf_".$_payload["data"]["key"]][ "user_val" ] )
            		&& isset( WC()->cart->cart_contents[$_payload['cart_item_key']]["wccpf_".$_payload["data"]["key"]][ "ftype" ] ) ) {
            				/* To remove old pricing rule label on cart */
            				$pricing_rules = WC()->cart->cart_contents[$_payload['cart_item_key']]["wccpf_".$_payload["data"]["key"]];
            			...
            			WC()->cart->cart_contents[$_payload['cart_item_key']]["wccpf_".$_payload["data"]["key"]][ "user_val" ] = $saveval;
            			$return_value = WC()->cart->cart_contents[$_payload['cart_item_key']]["wccpf_".$_payload["data"]["key"]][ "user_val" ];
            	} else {
            		WC()->cart->cart_contents[$_payload['cart_item_key']]["wccpf_".$_payload["data"]["key"]] = $saveval;
            		$return_value = WC()->cart->cart_contents[$_payload['cart_item_key']]["wccpf_".$_payload["data"]["key"]];
            	}
    ...
    function validate_wccpf($_prod_id, $_key, $_value, $cart_key, $_variation_id = 0) {
        ...
        foreach ($this->product_fields as $val) {
            if(isset($val["wccpf_".$_key ])){
                $fieldc = $val["wccpf_".$_key];
            }
            if( $this->fields_cloning == "yes" ){
                if(isset($val["wccpf_".$_key."_1"])){
                    $fieldc = $val["wccpf_".$_key."_1"];
                }
            }
        }
        foreach ($this->admin_fields as $avalue) {
            if(isset($avalue["wccaf_".$_key])){
                $fieldac = $avalue["wccaf_".$_key];
            }
            if( $this->fields_cloning == "yes" ){
                if(isset($val["wccaf_".$_key."_1"])){
                    $fieldac = $val["wccaf_".$_key."_1"];
                }
            }
        }
        if ($fieldc != null) {
            ...
            if ($field ["required"] == "yes" || $field ["type"] == "file") {
                if ($field ["type"] != "file") {
                    $res = wcff()->validator->validate_immediate($_prod_id, $field, $_key, $_value);
                } else {
        ...
        if ($fieldac != null) {
            ...
            if ($afield ["show_on_product_page"] == "yes" && $afield ["required"] == "yes") {
                $res = wcff()->validator->validate_immediate($_prod_id, $afield, $_key, $_value);
            }
            if (!$res) {
                $is_passed = false;
                $msg = $afield ["message"];
