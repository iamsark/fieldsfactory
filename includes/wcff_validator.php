...
class wcff_validator {
    ...
    public function validate($_pid, $_passed) {
        ...
        $this->quantity = intval($_REQUEST["quantity"]);
        ...
        error_log("validation result : ". $this->passed);
        return $this->passed;
    }
     ...
     * 
     */
    public function validate_helper($_prod_id, $_key, $_value, $cart_key, $_variation_id = 0) {
            $is_passed = true;
            $is_admin  = false;
            $wccpf_options = wcff()->option->get_options ();
