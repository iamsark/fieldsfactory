...
class wcff_builder {
    	...
    	<div class="wcff_fields_factory wcff_fields_factory_config_wrapper" action="POST" style="display: none;">
        	<div class="wcff_fields_factory_config_container">
            	...
            	<div class="wcff-factory-tab-header">
        			...
        			<a href=".wcff-factory-tab-fields-rules"><?php _e( 'Fields Rules', 'wc-fields-factory' ); ?></a>
        			<?php endif; ?>
     ...
     *
     */
    private function build_textarea_field($_meta, $_ptype = "wccpf", $_class = "", $_index, $_show_as_value = "no", $_readonly = "", $_cloneable = "", $_wrapper = true, $_has_pricing_rules = "no") {
        $html = '';
        ...
        if ($_show_as_value == "no") {
            $html = '<textarea data-has_field_rules="'.$has_field_rules.'" data-has_pricing_rules="'.$_has_pricing_rules.'" data-fkey="'. $_meta["key"] .'" class="' . $_ptype . '-field ' . $_class . '" name="' . esc_attr($_meta["key"] . $_index) . '" ' . $placeholder . ' ' . $rows . ' ' . $maxlength . ' data-field-type="'. $_meta["type"] .'" ' . $_ptype . '-pattern="mandatory" ' . $_ptype . '-mandatory="' . $_meta["required"] . '" ' . $_cloneable . ' ' . $_readonly . ' >' . (($_ptype != "wccaf") ? esc_html($_meta["default_value"]) : esc_html($_meta['value'])) . '</textarea>';
        } else {
            /*
             * Show the raw value instead of as a field
             * Used for the Admin Field showing on product page
     ...
     *
     */
    private function build_checkbox_field($_meta, $_ptype = "wccpf", $_class = "", $_index, $_show_as_value = "no", $_readonly = "", $_cloneable = "", $_wrapper = true, $_has_pricing_rules = "no") {
        $html = '';
        $defaults = array();        
        $has_field_rules = isset( $_meta["field_rules"] ) && is_array( $_meta["field_rules"] ) && count( $_meta["field_rules"] ) != 0 ? "yes" : "no";
        if ($_show_as_value == "no") {
            ...
            foreach ($choices as $choice) {
                ...
                if (count($key_val) == 2) {
                    ...
                    /* For admin field, we don't need <li></li> wrapper */
                    $html .= (($_ptype != "wccaf") ? '<li>' : '') . '<label class="wcff-option-wrapper-label"><input type="checkbox" data-has_field_rules="'.$has_field_rules.'" data-has_pricing_rules="'.$_has_pricing_rules.'" data-fkey="'. $_meta["key"] .'" class="' . $_ptype . '-field ' . $_class . '" name="' . esc_attr($_meta["key"] . $_index) . '[]" value="' . esc_attr(trim($key_val[0])) . '" ' . $attr . ' data-field-type="'. $_meta["type"] .'" ' . $_ptype . '-pattern="mandatory" ' . $_ptype . '-mandatory="' . $_meta["required"] . '" ' . $_readonly . ' /> ' . esc_attr(trim($key_val[1])) . '</label>' . (($_ptype != "wccaf") ? '</li>' : '');
                }
            }
            /* For admin field, we don't need <ul> wrapper */
            $html .= ($_ptype != "wccaf") ? '</ul>' : '';
    ...
    private function build_image_field($_meta) {
        ...
        $html .= '<input type="hidden" id="' . esc_attr($_meta["key"]) . '" name="' . esc_attr($_meta["key"]) . '" value="" />';
    ...
    private function built_field_wrapper($_html, $_meta, $_ptype, $_index) {
        ...
        if ($_ptype != "wccaf" && $_ptype != "wcccf" ) {
            ...
            if (has_action('wccpf_before_field_rendering') && has_action('wccpf_after_field_rendering')) {
                ...
                /* Label location */
                $label_alignment = isset($_meta["label_alignment"]) ? $_meta["label_alignment"] : "left";
