...
class wcff_cart_data_renderer {
    ...
    public function render_fields_data($_cart_data, $_cart_item = null) {
        ...
        if( isset( $this->cart_item['variation_id'] ) && $this->cart_item['variation_id'] != 0 && !empty( $this->cart_item['variation_id'] ) ){
            $wccvf_posts = wcff()->dao->load_fields_groups_for_product($this->cart_item['variation_id'], 'wccvf', $template, "any");
        ...
        if ($this->is_cloning_enabled == "no") {
            ...
            $this->render_fields($this->product_field_groups);
            /* Render Admin Fields that has been configured to show on Product Page */
            $this->render_fields($this->admin_field_groups);
        } else {
            $quantity = intval($this->cart_item["quantity"]);     
            for ($i = 1; $i <= $quantity; $i++) {
                /* Persist Product Fields */
                $this->render_fields($this->product_field_groups, $i);
                /* Persist Admin Fields that has been configured to show on Product Page */
                $this->render_fields($this->admin_field_groups, $i);
            }
        }
            ...
            if (count($group["fields"]) > 0) {
                foreach ($group["fields"] as $field) { 
                    ...
                    if ($field["visibility"] == "yes" && isset($this->cart_item[$fname])) {
                        $this->render_data($field, $this->cart_item[$fname]);
                    }                    
                }
            }
        }
