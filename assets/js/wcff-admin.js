...
(function($) {	
	...
	var wcff = function() {		
		...
		/* Used to render the fields on the layout designer */
		this.layoutFieldIndex = 0;
		...
		this.initialize = function() {
			...
			if (typeof wcff_var !== 'undefined' && wcff_var.post_type && wcff_var.post_type === "wccvf") {
				$("#toplevel_page_edit-post_type-wccpf > ul.wp-submenu > li").removeClass("current")
				$("#toplevel_page_edit-post_type-wccpf > ul.wp-submenu > li:nth-child(7)").addClass("current");
			}
		...
		this.registerEvents = function() {	
			...
			$(document).on("click", "div.variation-config-ghost-back", function() {
				$("ul.wcff-variation-config-popup").hide();
				$(this).remove();
			})
			
			$(document).on("click", "ul.wcff-variation-config-popup", function(e) {
				e.stopPropagation();
			});
			...
			$(document).on("click", "div.dropped > a.delete-field", this, function(e) {
				...
				var i = 0,
					j = 0,					
					fkey = $(this).parent().attr("data-fkey");
				...
				for (i = 0; i < e.data.layout.rows.length; i++) {					
					for (j = 0; j < e.data.layout.rows[i].length; j++) {
						if (e.data.layout.rows[i][j] == fkey) {
							e.data.layout.rows[i].splice(j, 1);
							if (e.data.layout.rows[i].length == 0) {
			...
			$(document).on("click", "div.wcff-meta-row > table.wcff_table", this, function(e) {
				...
				e.data.activeRow.addClass("opened active");				
				/* Check the field's configuration view, whether its already attached */
				if (e.data.activeRow.find("div.wcff_fields_factory").length > 0) {
					var me = e.data;					
					e.data.activeRow.find("div.wcff_fields_factory").toggle("slow", "swing", function() {
						if (!me.activeRow.find("div.wcff_fields_factory").is(":visible")) {						
							me.activeRow.removeClass("active");
							me.activeRow.find("input[name=wcff-field-type-meta-label-temp]").parent().html($("input[name=wcff-field-type-meta-label-temp]").val());		
							me.activeRow = null;
						} else {							
							me.activeRow.find(".wcff-field-label").html('<input type="text" name="wcff-field-type-meta-label-temp" value="'+ me.activeRow.find(".wcff-field-label").text() +'">');
						}										
					});				
				} else {
					e.data.mask.doMask($("#wcff-fields-set"));
					e.data.activeRow.find(".wcff-field-label").html('<input type="text" name="wcff-field-type-meta-label-temp" value="'+ e.data.activeRow.find(".wcff-field-label").text() +'">');
					/* Configuration view not attached, so fetch it from the server */					
					e.data.prepareRequest("GET", "field", {key: e.data.activeRow.attr("data-key"), type: e.data.activeRow.attr("data-type")}, e.data.activeRow);
					e.data.dock();
				}
			});
			...
			$(document).on("click", "div.wcff-factory-tab-header > a", this, function(e) {	 e.preventDefault();			
			...
			$(document).on("click", ".wcff-variation-config-search-field", this, function(e) {
				...
				var vbox = $("#wcff-variation-config-variation-select");
				/* If it is variation search then make sure the product search box shown and has product selected */
				if ($(this).attr("data-type") == "product_variation" && pbox.find("> li").length > 1) {
					if (!pbox.is(":visible")) {
						pbox.fadeIn("normal");
					}
				...
				} else if ($(this).attr("data-type") == "product_variation" && pbox.find("> li").length == 0) {
					alert("Please choose a Product First");
				...
				if ($(this).attr("data-type") == "product_variation") {
					e.data.prepareRequest("GET", "search", {"search": $(this).val(), "post_type": $(this).attr("data-type"), "parent": pbox.find("a.selected").attr("data-id")}, $(this));
				} else {
					e.data.prepareRequest("GET", "search", {"search": $(this).val(), "post_type": $(this).attr("data-type")}, $(this));					
				}	
				e.data.dock();		
			...
			$(document).on("keydown", ".wcff-variation-config-search-field", this, function(e) {
				...
				$(this).parent().find(">img").show();
				if ($(this).attr("data-type") == "product_variation") {
					req = e.data.prepareSearchRequest("GET", "search", {"search": $(this).val(), "post_type": $(this).attr("data-type"), "parent": pbox.find("a.selected").attr("data-id")}, $(this));
				} else {
					req = e.data.prepareSearchRequest("GET", "search", {"search": $(this).val(), "post_type": $(this).attr("data-type")}, $(this));
				}				
			...
			$(document).on("click", "ul.wcff-variation-config-popup a", this, function(e) {
				...
				if ($(this).closest("ul").attr("data-type") == "product") {
					...
					e.data.prepareRequest("GET", "search", {"search": $("#wcff-variation-config-variation-search").val(), "post_type": "product_variation", "parent": $(this).attr("data-id")}, $(this));
					e.data.dock();
				} else {
			...
			$(document).on("click", "a.wccvf-grid-group-remove-btn", this, function(e) {
				...
				e.data.prepareRequest("DELETE", "mapping", {"wccvf_id": $(this).attr("data-gid"), "endpoint": $(this).attr("data-vid")}, $(this));
				e.data.dock();		
			});
		...
		this.handleSearch = function(_req, _res) {
			if (_req.payload && _req.payload["post_type"]) {
				...
				if (popup) {
					popup.show();					
					popup.closest("table").find("img").hide();					
					for (i = 0; i < _res.payload.length; i++) {
						html += '<li><a href="" data-id="'+ _res.payload[i].id +'">'+ _res.payload[i].title +'</a></li>';
					}					
					popup.html(html);					
				}
		...
		this.handleVariationFieldsMap = function() {
			var i = 0,
				...
				selectedVariations = $("#wcff-variation-config-variation-select a.selected");
			...
			selectedGroups.each(function() {
				...
				for (i = 0; i < variations.length; i++) {
					rule = {};
					rule["context"] = "product_variation";
					rule["logic"] = "==";
					rule["endpoint"] = variations[i];
					rules.push(rule);
				}			
		...
		this.dropNewField = function() {
			...
			fieldRow = '<div class="wcff-meta-row active wcff-field-config-drawer-opened" data-key="" data-type="'+ field_type +'">';
			...
			fieldRow += '<label class="wcff-field-label" data-key=""><input type="text" name="wcff-field-type-meta-label-temp" class="wcff-field-type-meta-label-temp" value=""></label></td>';
		...
		this.prepareConfigWidget = function() {
			...
			this.target.find(".wcff-field-types-meta-body div.wcff-field-types-meta").each(function() {
				if (me.activeField[$(this).attr("data-param")]) {
					...
					if ($(this).attr("data-type") === "checkbox") {
						...
						if (choices) {
							for (i = 0; i < choices.length; i++) {					
								me.target.find("input.wcff-field-type-meta-"+ $(this).attr("data-param") +"[value='"+ choices[i] +"']" ).prop('checked', true);
							}
						}					
					} else if ($(this).attr("data-type") === "radio") {
						me.target.find(".wcff-field-type-meta-"+ $(this).attr("data-param") +"[value='"+ me.activeField[$(this).attr("data-param")] +"']").prop('checked', true);				
						me.target.find(".wcff-field-type-meta-"+ $(this).attr("data-param") +"[value='"+ me.activeField[$(this).attr("data-param")] +"']" ).trigger( "change" );
					} else {
						if ($(this).attr("data-type") !== "html") {
							me.target.find(".wcff-field-type-meta-"+$(this).attr("data-param")).val(me.unEscapeQuote(me.activeField[$(this).attr("data-param")]));
						}						
					}
					/* Load locale related fields */
					if (me.activeField["locale"]) {
						for (i = 0; i < wcff_var.locales.length; i++) {
							if (me.target.find("[name=wcff-field-type-meta-"+ $(this).attr("data-param") + "-" + wcff_var.locales[i] + "]" ).length > 0) {
								...
								if (me.activeField["locale"][wcff_var.locales[i]] && me.activeField["locale"][wcff_var.locales[i]][$(this).attr("data-param")]) {
									me.target.find("[name=wcff-field-type-meta-"+ $(this).attr("data-param") + "-" + wcff_var.locales[i] + "]" ).val(me.activeField["locale"][wcff_var.locales[i]][$(this).attr("data-param")]);
								}						
							}					
						}
					}
		...
		this.fetchFieldConfig = function() {
			...
			if (this.activeRow) {				
				...
				if (payload.type === "datepicker") {					
					min_max_hours = "0:23";
					min_max_minutes = "0:59";
					isTimePicker = this.activeRow.find("input[name=wcff-field-type-meta-timepicker]:checked").val();
		...
		this.loadFieldList = function(_payload) {
			...
			if (_payload.fields) {				
				...
				if ($.isEmptyObject(this.layout) || (Array.isArray(this.layout) && this.layout.length == 0)) {
					this.layout = {};
					this.layout["rows"] = [[]];
					this.layout["columns"] = {};
				}
				...
				/* Initialize the layout meta, if it is empty */
		...
		this.getLayoutField = function() {
			if (this.layoutFieldIndex < this.layoutFieldsKeys.length && this.fields[this.layoutFieldsKeys[this.layoutFieldIndex]]) {
				var _meta = this.fields[this.layoutFieldsKeys[this.layoutFieldIndex]];
				_meta["label_alignment"] = $("input[name=wcff_label_alignment_radio]:checked").val();
				this.prepareRequest("GET", "render_field_for_designer", {meta: _meta}, null);
		...
		this.renderLayoutField = function(_payload) {
			if (this.fields[this.layoutFieldsKeys[this.layoutFieldIndex]]) {
				var container = $('div.dropped[data-fkey='+ this.layoutFieldsKeys[this.layoutFieldIndex] +']');
				if (container.length > 0) {
					container.html(_payload);
					container.append($('<a href="#" class="delete-field" title="Remove">X</a>'));
				}
				/* Fetch next field */
				this.layoutFieldIndex++;
				this.getLayoutField();
			}		
		};
		...
		this.responseHandler = function() {
			if(this.request.context === "product" ||
				...
				this.dirtyFields[this.activeField.id] = this.activeField;				
			...
			} else if (this.request.context === "render_field_for_designer") {
				...
				/* disable the ajax lock */
				this.ajaxFlaQ = true;
				this.mask.doMask(this.mapping_grid.gridTable);
